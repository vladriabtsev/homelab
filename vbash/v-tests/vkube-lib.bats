#!/usr/bin/env bats
# k8s bash tests: ./bats/bin/bats ./vkube-lib.bats --filter-status failed
# k8s bash tests: ./bats/bin/bats ./vkube-lib.bats --filter-tags tag:test
# k8s bash tests: ./bats/bin/bats ./vkube-lib.bats --filter-tags tag:core
# k8s bash tests: ./bats/bin/bats ./vkube-lib.bats --filter-tags tag:storage
# k8s bash tests: ./bats/bin/bats ./vkube-lib.bats --filter-tags tag:storage-speed
...
#region secret
  ...
  @test "vkube-lib.secret-create-from-folder: without namespace" {
    run vkube-lib.secret-create-from-folder
    #echo "output=$output"
    assert_failure
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-folder: without secret name" {
    run vkube-lib.secret-create-from-folder test
    #echo "output=$output"
    assert_failure
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-folder: without secret folder" {
    run vkube-lib.secret-create-from-folder test test-secret
    #echo "output=$output"
    assert_failure
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-folder: from not existing disk folder" {
    run vkube-lib.secret-create-from-folder test test-secret ~/.test-not-exists
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-folder: from empty disk folder" {
    run vkube-lib.secret-create-from-folder test test-secret ~
    #echo "output=$output" >&3
    assert_failure
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-folder: from disk folder with username and password" {
    # Required:
    # ~/.test/username.txt # with line 'username=test-user'
    # ~/.test/password.txt # with line 'password=test-password'
    run vkube-lib.secret-create-from-folder test test-secret ~/.test
    #echo "output=$output" >&3
    assert_success
    assert_output --partial "secret/test-secret created"
  }
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-folder: from disk folder" {
    # Required:
    # ~/.test/username.txt # with line 'username=test-user'
    # ~/.test/password.txt # with line 'password=test-password'
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-pass-folder: from not existing path in 'pass' password store" {
    run vkube-lib.secret-create-from-pass-folder test test-secret not-existing-store-folder
    #echo "output=$output" >&3
    assert_failure
  ...
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-pass-folder: from path in 'pass' password store" {
    # Required:
    # pass insert test/username.txt # enter password 'test-user'
    # pass insert test/password.txt # enter password 'test-password'
    run vkube-lib.secret-create-from-pass-folder test test-secret test
    #echo "output=$output" >&3
    assert_success
  }
  # TODO tests without password request
  #bats test_tags=tag:secret
  @test "vkube-lib.secret-create-from-pass-folder: from password manager" {
    # Required:
    # ~/.test/username.txt # with line 'username=test-user'
    # ~/.test/password.txt # with line 'password=test-password'
    run kubectl delete secret test-secret -n test --ignore-not-found=true
...
@test "k3d core installation" {
  ...
  #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster install --core" >&3
  run ../vkube --cluster-plan k3d-test --trace cluster install --core
  sleep 20
  assert_success
  ...
  run vkube-lib.is-namespace-exist "csi-nfs"
  [ "$status" -eq 1 ]
  run vkube-lib.is-namespace-exist "csi-smb"
  [ "$status" -eq 1 ]
  run vkube-lib.is-namespace-exist "synology-csi"
  [ "$status" -eq 1 ]
}
...
@test "velero installation" {
  ...
  run ../vkube --cluster-plan k3d-test --trace cluster install --velero
  assert_success
...
#region storage install and uninstall
  ...
  @test "storage: install-uninstall local" {
    ...
    #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster install --local" >&3
    run ../vkube --cluster-plan k3d-test --trace cluster install --local
    sleep 10
    assert_success
    run verify "there is 1 storageclass named 'local-storage'"
	  [ "$status" -eq 0 ]

    #vkube-lib.-internal-storage-speed-test "local-path"
    run ../vkube --cluster-plan k3d-test --trace cluster storage-speed-test "local-path" --csi-driver-nfs
  }
  # bats test_tags=tag:storage-separate
  # https://rudimartinsen.com/2024/01/09/nfs-csi-driver-kubernetes/
  # https://github.com/kubernetes-csi/csi-driver-nfs
  @test "storage: install-uninstall nfs" {
    #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster install --csi-driver-nfs" >&3
    run ../vkube --cluster-plan k3d-test --trace cluster install --csi-driver-nfs
    #echo '         Testing...' >&3
    sleep 60
    ...
    #vkube-lib.-internal-storage-speed-test "office-csi-driver-nfs-retain"
    run ../vkube --cluster-plan k3d-test --trace cluster storage-speed-test "office-csi-driver-nfs-retain" --csi-driver-nfs
  ...
  @test "storage: install-uninstall smb" {
    #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster install --csi-driver-smb" >&3
    run ../vkube --cluster-plan k3d-test --trace cluster install --csi-driver-smb
    #echo '         Testing...' >&3
    sleep 60
    ...
    #vkube-lib.-internal-storage-speed-test "office-csi-driver-smb-tmp"
    run ../vkube --cluster-plan k3d-test --trace cluster storage-speed-test "office-csi-driver-smb-tmp" --csi-driver-nfs
  ...
  @test "storage: install-uninstall synology-csi" {
    #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster install --csi-synology" >&3
    run ../vkube --cluster-plan k3d-test --trace cluster install --csi-synology
    #echo '         Testing...' >&3
    sleep 30
    DETIK_CLIENT_NAMESPACE="synology-csi"
    #echo '         Testing synology-csi-node' >&3
    run try "at most 10 times every 30s to get pods named '^synology-csi-node' and verify that 'status' is 'running'"
    assert_success
    #echo '         Testing synology-csi-controller' >&3
    run try "at most 10 times every 30s to get pods named '^synology-csi-controller' and verify that 'status' is 'running'"
    assert_success

    #vkube-lib.-internal-storage-speed-test "office-synology-csi-nfs-retain"
    run ../vkube --cluster-plan k3d-test --trace cluster storage-speed-test "office-synology-csi-nfs-retain" --csi-driver-nfs
    #vkube-lib.-internal-storage-speed-test "office-synology-csi-smb-tmp"
    run ../vkube --cluster-plan k3d-test --trace cluster storage-speed-test "office-synology-csi-smb-tmp" --csi-driver-nfs

    # # https://github.com/SynologyOpenSource/synology-csi/pull/85
    # storage="office-synology-csi-iscsi-tmp"
    # #echo "      Step $[step=$step+1]. vkube-lib.storage-speedtest-job-create storage-speedtest $storage ReadWriteOnce" >&3
    ...
    # run vkube-lib.storage-speedtest-job-create storage-speedtest $storage ReadWriteOnce
    # assert_success
    # #echo '        Testing...' >&3
    # sleep 15
  ...
  @test "storage: install-uninstall longhorn" {
    ...
    if kubectl get ns/longhorn-system; then
      run ../vkube --cluster-plan k3d-test --trace cluster storage-speed-test "longhorn" --longhorn
    else
      skip "Longhorn is not installed. If it k3d then probably because open-iscsi is not supported in k3d"
    fi

    #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster install --longhorn" >&3
    run ../vkube --cluster-plan k3d-test --trace cluster install --longhorn
    ...
    # #echo "      Step $[step=$step+1]. ../vkube --cluster-plan k3d-test --trace cluster uninstall --longhorn" >&3
    # run ../vkube --cluster-plan k3d-test --trace cluster uninstall --longhorn
    # sleep 10
    # assert_success
  }
...
@test "storage-classes: regenerate" {
  #echo "      Step $[step=$step+1]. ../vkube --force --cluster-plan k3d-test --trace cluster install --storage-classes-only" >&3
  run ../vkube --force --cluster-plan k3d-test --trace cluster install --storage-classes-only
  sleep 10
  assert_success
  run verify "there is 1 storageclass named 'local-storage'"
}
...
@test "k3d general storage installation" {
  #echo "      Step $[step=$step+1]. ../vkube --force --cluster-plan k3d-test --trace cluster install --storage" >&3
  run ../vkube --force --cluster-plan k3d-test --trace cluster install --storage
  sleep 10
  assert_success
...
#region general storage speed tests
  ...
  @test "../vkube --cluster-plan k3d-test cluster storage-speed 'local-path'" {
    run ../vkube --cluster-plan k3d-test cluster storage-speed 'local-path' # >&3
    #echo "output=$output" >&3
  }

  # bats test_tags=tag:storage-speed-one
  @test "../vkube --cluster-plan k3d-test cluster storage-speed 'office-csi-driver-nfs-retain'" {
    run ../vkube --cluster-plan k3d-test cluster storage-speed 'office-csi-driver-nfs-retain'
    assert_success
    #echo "output=$output" >&3
  }

  # bats test_tags=tag:storage-speed
  @test "../vkube --cluster-plan k3d-test cluster storage-speed 'office-csi-driver-smb-tmp'" {
    run ../vkube --cluster-plan k3d-test cluster storage-speed 'office-csi-driver-smb-tmp'
    assert_success
    #echo "output=$output" >&3
  }

  # bats test_tags=tag:storage-speed
  @test "../vkube --cluster-plan k3d-test cluster storage-speed 'office-synology-csi-nfs-retain'" {
    run ../vkube --cluster-plan k3d-test cluster storage-speed 'office-synology-csi-nfs-retain'
    assert_success
    #echo "output=$output" >&3
  }

  # bats test_tags=tag:storage-speed
  @test "../vkube --cluster-plan k3d-test cluster storage-speed 'direct-pv-del'" {
    skip "not implemented yet"
    run ../vkube --cluster-plan k3d-test cluster storage-speed 'direct-pv-del'
    assert_success
    #echo "output=$output" >&3
  }

  # bats test_tags=tag:storage-speed
  @test "../vkube --cluster-plan k3d-test cluster storage-speed 'longhorn'" {
    if ! kubectl get ns/longhorn-system; then
      skip "Longhorn is not installed"
    fi
    run ../vkube --cluster-plan k3d-test cluster storage-speed 'longhorn'
    assert_success
    #echo "output=$output" >&3
  }
  ...
  #   vkube-lib.-internal-storage-speed-test "office-synology-csi-smb-tmp"
  #   if kubectl get ns/longhorn-system; then
  #     vkube-lib.-internal-storage-speed-test "longhorn"
  #   fi
  # }
...
#   ___ver=$(vkube-lib.get-pod-image-version default test-minio minio)
#   [ "$status" -eq 0 ]
#   [ "$___ver" == "4.0.18" ]
# }
...
@test "synology-csi installation integration tests" {
  ...
  __synology_ver=$(vkube-lib.get-pod-image-version synology-csi synology-csi-controller csi-plugin)
  [ "$status" -eq 0 ]
...
@test "busybox installation integration tests" {
  ...
  ___ver=$(vkube-lib.get-pod-image-version default test-busybox busybox)
  [ "$status" -eq 0 ]
  [ "$___ver" == "1.37.0" ]
...
# bats test_tags=tag:app
@test "install unknown cluster app" {
  run ../vkube --cluster-plan k3d-test --trace cluster install --app unknown
  #echo "output=$output"
  assert_failure
  assert_output --partial "Application name 'unknown' is not found in 'cluster_apps:' section of cluster plan '"
  assert_output --partial "## C A L L   T R A C E ##"
}
# bats test_tags=tag:app
@test "delete unknown cluster app" {
  run ../vkube --cluster-plan k3d-test --trace cluster delete --app unknown
  #echo "output=$output"
  assert_failure
  assert_output --partial "Application name 'unknown' is not found in 'cluster_apps:' section of cluster plan '"
  assert_output --partial "## C A L L   T R A C E ##"
...
# bats test_tags=tag:app
@test "install cluster app msql2025" {
  run kubectl delete namespace msql2025 --ignore-not-found
  assert_success

  run ../vkube --cluster-plan k3d-test --trace cluster install --app msql2025
  sleep 20
  assert_success

  # https://github.com/bats-core/bats-assert#partial-matching
  sleep 60
  DETIK_CLIENT_NAMESPACE="msql2025"
  run verify "there are 1 pods named '^msql2025'"
  assert_success
  run try "at most 10 times every 30s to get pods named '^msql2025' and verify that 'status' is 'running'"
  assert_success
  run verify "'status' is 'running' for pods named '^msql2025'"
  assert_success
  [ "$status" -eq 0 ]
}
# bats test_tags=tag:app
@test "delete cluster app msql2025" {
  run ../vkube --cluster-plan k3d-test --trace cluster delete --app msql2025
  sleep 20
  assert_success
...
# bats test_tags=tag:app
@test "cluster all apps installation" {
  if k3d version &> /dev/null; then
    run k3d cluster delete test
  fi
  run ../vkube --cluster-plan k3d-test --trace cluster install --app
  sleep 20
  assert_success

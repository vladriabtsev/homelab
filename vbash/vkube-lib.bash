...
function vkube-lib.get-pod-image-version() {
...
# function vkube-lib.is-deployment-installed() {
#   [[ -z $1 ]] && vlib.error-printf "Missing namespace parameter"
...
#   fi
# }
function vkube-lib.fail-if-already-deployed() {
  [[ -z $1 ]] && vlib.error-printf "Missing namespace parameter"
...
function vkube-lib.cluster-plan-read() {
...
#region secret
function vkube-lib.check-dir-data-for-secrets-with-trace() {
...
function vkube-lib.check-dir-data-for-secrets() {
  [ -z "$1" ] && err_and_exit "Missing folder path \$1 parameter."
  if [ -d "$1" ]; then
...
function vkube-lib.check-pass-data-for-secrets-with-trace() {
...
function vkube-lib.secret-create {
  ...
  if [[ -n $3 ]]; then
    vkube-lib.secret-create-from-folder $1 $2 $3
  else
    vkube-lib.secret-create-from-pass-folder $1 $2 $4
  fi
}
function vkube-lib.secret-create-from-folder {
  #############################
  # Create kubernetes secret
  #  $1 - kubernetes namespace where secret will be created
  #  $2 - kubernetes secret name
  ...
  vkube-lib.check-dir-data-for-secrets-with-trace "$path"
  vkube-lib.namespace-create-if-not-exist $1
...
function vkube-lib.secret-create-from-pass-folder {
  #############################
  # Create kubernetes secret
  #  $1 - kubernetes namespace where secret will be created
  #  $2 - kubernetes secret name
  ...
  [ -z "$2" ] && err_and_exit "Missing secret name \$2 parameter."
  [ -z "$3" ] && err_and_exit "Missing secret folder path \$3 parameter."
  vkube-lib.check-pass-data-for-secrets-with-trace "$3"
  local username
...
#endregion secret
function vkube-lib.get-node-admin-password() {
  local password
...
function vkube-lib.install-kubectl() {
  # Check kubectl
...
function vkube-lib.install-helm() {
  # Check helm
...
function vkube-lib.install-k3-tools() {
...
function vkube-lib.install-velero-get() {
...
function vkube-lib.install-velero() {
  if [[ $velero_use -eq 1 ]]; then
    ...
    if ! command -v velero version > /dev/null 2> /dev/null; then # not installed
      if [ -n "$user_pc_stop_if_tools_need_install" ] && [ $user_pc_stop_if_tools_need_install -eq 1 ]; then
        ...
        vkube-lib.install-velero-get "$ver_curr"
      ...
      if [[ -n "$ver_curr" ]] && [[ -n "$velero_ver" ]] && [[ "$velero_ver" != "$ver_curr" ]]; then
        ...
        vkube-lib.install-velero-get "$velero_ver"
      fi
    fi
  fi
}
function vkube-lib.install_tools() {
  ...
  vkube-lib.install-jq
  vkube-lib.install-kubectl

  vkube-lib.install-helm
  vkube-lib.install-k3-tools
  vkube-lib.install-velero
...
function _install-app() {
  [ -z "$1" ] && err_and_exit "Missing \$1 - app name"
  [ -z "$2" ] && err_and_exit "Missing \$2 - app node"
  [ -z "$3" ] && err_and_exit "Missing \$3 - install/delete"
  h2 "Application: '$1'"
  readarray _app_commands < <(echo $2 | yq -o=j -I=0 ".$3-commands[]")
  local _command_no=0
  local _app_command2
  for _app_command in "${_app_commands[@]}"; do
    #vlib.trace "_app_command=${_app_command}"
    #vlib.trace "_app_command length=${#_app_command}"
    local _len1=${#_app_command}
    local _len=$((_len1 - 3))
    #vlib.trace "_len=${_len}"
    _app_command="${_app_command:1:$_len}"
    #vlib.trace "_app_command=${_app_command}"
    _app_command2="${_app_command//##vkube-data##/$vkube_data_folder}"
    vlib.trace "_app_command2=${_app_command2}"
    local _o
    local _exit_code
    if [[ "${_app_command:0:13}" == "##create-ns##" ]]; then
      #inf "Command $((++_command_no)):\n"
      run vkube-lib.namespace-create-if-not-exist "${_app_command:14}"
    else
      #inf "Command $((++_command_no)): '$_app_command'\n"
      #inf "Command $((++_command_no)):\n"
      #set -x
      _app_command="${_app_command//##vkube-data##/'^'}"
      run "_o=$(eval '$_app_command2' 2>&1 >/dev/null)#'$_app_command'" # Redirect stderr to variable, discard stdout
      _exit_code=$?
      #set +x
      [[ $_exit_code -ne 0 ]] && err_and_exit "App: '$1'. Exit code=$_exit_code. Message: $_o"
    fi
  done
}
function _install-applications() {
  [ -z "$1" ] && err_and_exit "Missing \$1 - install/delete"
  readarray _applications < <(yq -o=j -I=0 '.cluster_apps[]' < $cluster_plan_file)
  vlib.trace "cluster apps count=${#_applications[@]}"
  if [[ -n ${args[--app]} ]]; then
    vlib.trace "Only selected apps"
    local _sel_apps
    eval "_sel_apps=(${args[--app]:-})"
    for _sel_app in "${_sel_apps[@]}"; do
      _exec=0
      vlib.trace "_sel_app=${_sel_app}"
      for _app in "${_applications[@]}"; do
        vlib.trace "_app=${_app}"
        eval "$( yq '.[] | ( select(kind == "scalar") | "_app_" + key + "='\''" + . + "'\''")' <<<$_app)"
        vlib.trace "_app_name=${_app_name}"
        if [[ "$_app_name" == "$_sel_app" ]]; then 
          _install-app "$_app_name" "$_app" "$1"
          _exec=1
          break
        fi
      done
      if [ "$_exec" -eq 0 ]; then
        err_and_exit "Application name '$_sel_app' is not found in 'cluster_apps:' section of cluster plan '$cluster_plan_file' file. Check flag --app='$_sel_app' of 'vkube' script."
      fi
    done
  else
    vlib.trace "All apps"
...
function vkube-lib.install() {
  ...
  if [[ -n ${args[--storage-classes-only]} ]]; then
  ...
  vlib.trace-no-stack "install_core=$install_core"
  vlib.trace-no-stack "velero_use=$velero_use"
  vlib.trace-no-stack "install_apps=$install_apps"
  vlib.trace-no-stack "install_storage=$install_storage"
  ...
  vlib.trace "__install_all=$__install_all"

  if [[ ${install_core} -eq 1 ]]; then
    vkube-lib.install_tools
    ...
    case $kubernetes_type in
      k3s )
        ...
        if [ -n "$kube_vip_use" ] && [ "$kube_vip_use" -eq 1 ]; then
          ...
          if [[ $(kubectl get pods -lapp.kubernetes.io/name=kube-vip-ds,app.kubernetes.io/version=${kube_vip_ver} -n kube-system 2> /dev/null | wc -l) -eq 0 ]]; then
  ...
  if [[ $install_storage_at_least_one -eq 1 ]]; then
    install-storage
    local _excl_col=(provisioner)
    vkube-lib._storage_classes "$_excl_col"
  fi

  if [[ $install_apps -eq 1 ]]; then
    _install-applications install
  fi

  vkube-lib.get-cluster-containers
  ...
  run "line '$LINENO';vkube-lib.namespace-create-if-not-exist argocd"
  ...
  #fi
  run "line '$LINENO';vkube-lib.namespace-create-if-not-exist cattle-system"
  # helm repo add rancher-stable https://releases.rancher.com/server-charts/stable
...
function vkube-lib.delete() {
  ...
  if [[ ${install_core} -eq 1 ]]; then
    case $kubernetes_type in
      k3s )
        h2 "Delete K3s cluster '$cluster_name' with $amount_nodes nodes. Cluster plan from '$cluster_plan_file' file. (Line:$LINENO)"
        if [ -z "$SSH_AUTH_SOCK" ]; then
          eval "$(ssh-agent -s)" # start ssh agent
          ssh-add ~/.ssh/id_rsa
        fi
      ...
      ;;
      k3d )
        h2 "Delete K3d cluster '$cluster_name' with $amount_nodes nodes. Cluster plan from '$cluster_plan_file' file. (Line:$LINENO)"
        run "line '$LINENO';k3d cluster delete $cluster_name --wait"
        export KUBECONFIG=""
      ;;
      # k83 )
      #   vlib.trace "csi_synology_host_protocol_class_location=$csi_synology_host_protocol_class_location"
    ...
    inf "Kubernetes cluster '$cluster_name' is deleted on servers described in cluster plan YAML file '$cluster_plan_file'\n"
  fi
  ...
  if [[ $install_apps -eq 1 ]]; then
    _install-applications delete
  fi
}
function vkube-lib.get-storage-class-type() { # backup2-synology-csi-nfs-test
  # return container image version
...
function vkube-lib.csi-synology-install() {
  ...
  run "line '$LINENO';vkube-lib.namespace-create-if-not-exist $csi_synology_namespace"
  ...
  for csi_synology_host in "${csi_synology_hosts[@]}"; do
    ...
    for csi_synology_host_protocol in "${csi_synology_host_protocols[@]}"; do
      ...
      for csi_synology_host_protocol_class in "${csi_synology_host_protocol_classes[@]}"; do
        ...
        if [[ $(kubectl get storageclass $csi_synology_host_protocol_class_name -A 2> /dev/null | wc -l) -gt 0 ]]; then
...
"
          ...
          synology-csi-smb )
            ...
            run "line '$LINENO';vkube-lib.secret-create $csi_synology_namespace $secret_folder_name $csi_synology_host_protocol_secret_folder $csi_synology_host_protocol_secret_pass_folder"
            #region
            txt+="${separator}apiVersion: storage.k8s.io/v1 # line:${LINENO}
kind: StorageClass
metadata:
...
"
  ...
  if [[ -z ${args[--storage-classes-only]} ]]; then
    ...
    if [[ $(kubectl get pods -l app=controller,app.kubernetes.io/name=synology-csi -n $csi_synology_namespace 2> /dev/null | wc -l) -eq 0 ]]; then # not installed yet
      ...
      #run "line '$LINENO';vkube-lib.secret-create $csi_synology_namespace client-info-secret $csi_synology_folder_with_dsm_secrets/client-info.yml"
      ...
      #if ! test vkube-lib.is-app-ready "app=controller,app.kubernetes.io/name=synology-csi"; then
      #else
      #fi
    #     if is not ready
    #       delete
...
function vkube-lib.csi-synology-uninstall() {
  local deploy_k8s_version="v1.20"
...
function longhorn-storage-class-create() {
...
function longhorn-install() {
  if [[ -z ${args[--storage-classes-only]} ]]; then
    ...
    for node in "${nodes[@]}"; do
      ...
      if [[ ${#disks[@]} -gt 0 ]]; then
        node_root_password=$(vkube-lib.get-node-admin-password)
        node_disks 1
        node_disks 2
      fi
      ((i_node++))
...
function check-longhorn-exclusive-params() {
...
function install-storage() {
  if [[ -z ${args[--storage-classes-only]} ]]; then
    if [ -n "$velero_use" ] && [ "$velero_use" -eq 1 ]; then
      ...
      run "line '$LINENO';vkube-lib.namespace-create-if-not-exist $velero_namespace"
      ...
      local need_install
      curr_ver=$(vkube-lib.get-pod-image-version "$velero_namespace" true "velero/velero")
      ...
      curr_ver=$(vkube-lib.get-pod-image-version "$velero_namespace" false "velero/velero-plugin-for-aws")
    ...
    if [ -n "$csi_direct_pv_use" ] && [ "$csi_direct_pv_use" -eq 1 ]; then
      ...
      if [[ $(kubectl get pods -lapp=csi-nfs-controller,app.kubernetes.io/version=${csi_driver_nfs_ver:1} -n ${csi_driver_nfs_namespace} 2> /dev/null | wc -l) -eq 0 ]]; then
        run "line '$LINENO';vkube-lib.namespace-create-if-not-exist $csi_driver_nfs_namespace"
        run "line '$LINENO';helm repo add csi-driver-nfs https://raw.githubusercontent.com/kubernetes-csi/csi-driver-nfs/master/charts"
    ...
    if [ -n "$csi_driver_smb_use" ] && [ "$csi_driver_smb_use" -eq 1 ]; then
      ...
      if [[ $(kubectl get pods -lapp=csi-smb-controller,app.kubernetes.io/version=${csi_driver_smb_ver:1} -n ${csi_driver_smb_namespace} 2> /dev/null | wc -l) -eq 0 ]]; then
        run "line '$LINENO';vkube-lib.namespace-create-if-not-exist $csi_driver_smb_namespace"
        #       if [[ -n $csi_driver_smb_secret_folder ]]; then
        #         eval "csi_driver_smb_secret_folder=$csi_driver_smb_secret_folder"
        #         vkube-lib.vkube-lib.check-dir-data-for-secrets "$csi_driver_smb_secret_folder"
        ...
        #         vkube-lib.secret-create-from-pass-folder "$csi_driver_smb_secret_folder"
        #       else
    ...
    if [ -n "$nfs_subdir_external_provisioner_use" ] && [ "$nfs_subdir_external_provisioner_use" -eq 1 ]; then
      ...
      if [[ $(kubectl get pods -lapp=nfs-subdir-external-provisioner -n kube-system 2> /dev/null | wc -l) -eq 0 ]]; then
    ...
    vlib.trace "--storage-classes-only is set"
  fi
  # --storage-classes-only flag is handled by vkube-lib.csi-synology-install()
  if [ -n "$csi_synology_use" ] && [ "$csi_synology_use" -eq 1 ]; then
    vkube-lib.csi-synology-install
  fi
        ...
        case $storage_server_protocol_name in
          csi-driver-nfs )
            ...
            if [ -n "$csi_driver_nfs_use" ] && [ "$csi_driver_nfs_use" -eq 1 ]; then
              ...
              run vkube-lib.secret-create "$csi_driver_nfs_namespace" "$secret_name" "$storage_server_protocol_secret_folder" "$storage_server_protocol_secret_pass_folder"
              ...
              #   run "line '$LINENO';vkube-lib.secret-create-from-folder $csi_driver_nfs_namespace $secret_folder_name $storage_server_protocol_secret_folder"
              # elif [[ -z $storage_server_protocol_secret_pass_folder ]]; then
              #   run "line '$LINENO';vkube-lib.secret-create-from-pass $csi_driver_nfs_namespace $storage_server_protocol_secret_pass_folder $storage_server_protocol_secret_pass_folder"
              # fi
              ...
              #run "line '$LINENO';vkube-lib.secret-create csi-nfs $secret_folder_name $storage_server_protocol_secret_folder"
...
"
          ...
          csi-driver-smb )
            ...
            if [ -n "$csi_driver_smb_use" ] && [ "$csi_driver_smb_use" -eq 1 ]; then
              ...
              run vkube-lib.secret-create "$csi_driver_smb_namespace" "$secret_name" "$storage_server_protocol_secret_folder" "$storage_server_protocol_secret_pass_folder"

              # run "line '$LINENO';kubectl delete secret -n $csi_driver_smb_namespace $secret_folder_name --ignore-not-found=true"
              # # https://www.baeldung.com/ops/kubernetes-namespaces-common-secrets
              # if [[ -z $storage_server_protocol_secret_folder ]]; then
              #   secret_folder_name=$(basename "$storage_server_protocol_secret_folder")
              #   run "line '$LINENO';vkube-lib.secret-create-from-folder $csi_driver_smb_namespace $secret_folder_name $storage_server_protocol_secret_folder"
              # elif [[ -z $storage_server_protocol_secret_pass_folder ]]; then
              #   run "line '$LINENO';vkube-lib.secret-create-from-pass $csi_driver_smb_namespace $storage_server_protocol_secret_pass_folder $storage_server_protocol_secret_pass_folder"
              # fi
...
"
        #endregion
        [[ -z $storage_class ]] && err_and_exit "Storage class name is empty. Cluster plan: '$cluster_plan_file'. Storage server: '$storage_server_name'. Protocol name: '$storage_server_protocol_name'"
        if [[ $(kubectl get storageclass $storage_class -A 2> /dev/null | wc -l) -gt 0 ]]; then
          if [[ -n ${args[--force]} ]]; then
            run "line '$LINENO';kubectl delete storageclass $storage_class --wait --ignore-not-found=true"
          else
            err_and_exit "Storage class '$storage_class' already exists in cluster. Use --force flag to delete anyway."
...
function vkube-lib.storage-speedtest-job-create() {
...
function vkube-lib.-internal-storage-speed-test() {
  # $1 - storage driver
  local job_name=""
  ...
  #vlib.h1 "Step $[step=$step+1]. vkube-lib.storage-speedtest-job-create storage-speedtest $storage ReadWriteOnce"
  ...
  vkube-lib.storage-speedtest-job-create storage-speedtest "$job_name" ReadWriteOnce
  #sleep 15
...
function vkube-lib.storage-speed-test {
  #hl.blue "$((++install_step)). Storage class speed test. (Line:$LINENO)"
  vkube-lib.-internal-storage-speed-test "${args[--storage-class]}"
}
function vkube-lib.command-init() {
  vkube-lib.check-cluster-plan-path

  vkube-lib.cluster-plan-read
}

#region utils
function vkube-lib.get-cluster-containers() {
  # https://kubernetes.io/docs/tasks/access-application-cluster/list-all-running-container-images/
  h2 "Count container images in cluster '$cluster_name'"
  kubectl get pods --all-namespaces -o jsonpath="{.items[*].spec['initContainers', 'containers'][*].image}" |tr -s "[[:space:]]" "\n" |sort |uniq -c
}
function vkube-lib._storage_classes() {
  local _txt=""
  _storage_class_colmns=(name provisioner reclaim expansion bind)
  for _col in "${_storage_class_colmns[@]}"; do
    for _excl_col in "${_excl_cols[@]}"; do
      vlib.trace "_excl_col=${_excl_col}"
      if [[ "$_col" == "$_excl_col" ]]; then
        break
      fi
      case ${_col} in
        name )
          _txt="STORAGE CLASS NAME:.metadata.name"
        ;;
        provisioner )
          _txt="$_txt,PROVISIONER:.provisioner"
        ;;
        reclaim )
          _txt="$_txt,RECLAIM:.reclaimPolicy"
        ;;
        bind )
          _txt="$_txt,BIND MODE:.volumeBindingMode"
        ;;
        expansion )
          _txt="$_txt,EXPANSION:.allowVolumeExpansion"
        ;;
        * ) 
          err_and_exit "Unsupported column name '$_col'."
        ;;
      esac
    done
  done
  # kubectl get storageclasses -o 'custom-columns=CLASS NAME:.metadata.name'
  # kubectl get storageclasses -o 'custom-columns=PROV:.provisioner'
  # kubectl get storageclasses -o 'custom-columns=CLASS NAME:.metadata.name,PROV:.provisioner'
  vlib.trace "_txt=${_txt}"
  kubectl get storageclasses -o "custom-columns=$_txt"
}
function vkube-lib.get-cluster-storage-classes() {
  if [[ -n ${1} ]]; then
    vkube-lib._storage_classes "$1"
  elif [[ -n ${args[--no-headers]} ]]; then
    kubectl get storageclasses --no-headers
  elif [[ -n ${args[--no]} ]]; then
    local _excl_cols
    eval "_excl_cols=(${args[--no]:-})"
    vkube-lib._storage_classes _excl_cols
  else
...
#region app
function vkube-lib.app-check-releses() {
  if [ -z "$1" ]; then
...
function vkube-lib.app-install() {
  # https://jqlang.org/
  ...
  case ${1} in
    busybox )
      ...
      vkube-lib.app-check-releses 'https://raw.githubusercontent.com/docker-library/busybox/refs/heads/master/versions.json' "$_ver" "$_release"
    ;;
    * ) 
  ...
  vkube-lib.fail-if-already-deployed "${args[--namespace]}" "$_deployment"
  # if [[ $(vkube-lib.fail-if-already-deployed "${args[--namespace]}" "$_deployment") -eq 1 ]]; then
...
spec:
  ...
  for _storage_class in "${_storage_classes[@]}"; do
    ...
    __storage_type="$(vkube-lib.get-storage-class-type $_storage_class)"
...
"
  ...
  run "line '$LINENO';vkube-lib.namespace-create-if-not-exist ${args[--namespace]}"
  local _path
...
function vkube-lib.app-deployment-uninstall() {
  if [ -z "$1" ]; then

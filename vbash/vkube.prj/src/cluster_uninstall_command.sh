...
{
  inf "Uninstalling cluster first node. (Line:$LINENO)"
  if [[ $2 -eq 1 ]]; then
...
install_first_node()
{
  if [ $opt_install_new -eq 1 ]; then
    h2 "$((++install_step)). Bootstrap First cluster node $node_name($node_ip4). (Line:$LINENO)"
  else
    h2 "$((++install_step)). Remove cluster node $node_name($node_ip4). (Line:$LINENO)"
  fi
...
--tls-san $cluster_node_ip"
    inf "Install cluster first node. (Line:$LINENO)"
...
{
  if [ $opt_install_new -eq 1 ]; then
    h2 "$((++install_step)). Join cluster node $node_name($node_ip4). (Line:$LINENO)"
  else
    h2 "$((++install_step)). Remove cluster node $node_name($node_ip4). (Line:$LINENO)"
  fi
...
--server https://$cluster_node_ip:6443"
  ...
  if [ $opt_install_new -eq 1 ]; then
    ...
    #kubectl get nodes
    # sudo journalctl -xeu k3s.service # check cluster log on node
...
if [[ $opt_install_remove -eq 1 ]]; then
  h2 "Remove cluster cluster with $amount_nodes nodes. Cluster plan from '$cluster_plan_file' file. (Line:$LINENO)"
else
  h2 "Install cluster cluster with $amount_nodes nodes. Cluster plan from '$cluster_plan_file' file. (Line:$LINENO)"
fi
...
#read -p "Local cluster installation (y/n): " k3s_local_install
...
# Step 1: Bootstrap First cluster Node
mkdir ~/.kube
k3sup install \
  --ip $master1 \
  --user $user \
...
echo -e " \033[32;5mWaiting for cluster to sync and LoadBalancer to come online\033[0m"
